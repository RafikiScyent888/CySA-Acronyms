...
(function (global) {
  ...
  /* ONE KEY FOR THE WHOLE PROGRAM, DELIBERATELY.

     These sites are all served from the same GitHub Pages origin, so
     they share local storage. A student who chooses light on the
     Security+ acronyms should not have to choose it again on CySA+, or
     on any other tile. One choice, honoured everywhere.

     If a site ever moves to its own domain the key simply stops being
     shared and the toggle behaves as a per-site one, so there is no way
     for this to break. */
  var KEY = "cwp:theme";
  var LIGHT = "light";
  var DARK = "dark";
