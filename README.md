...
A free, static website for studying **175 acronyms** from a CompTIA CySA+ acronym/definition reference — as flashcards and four study games. No build step, no dependencies, no backend. Open `index.html` or host it on GitHub Pages.
...
All 175 acronym/expansion/definition entries were extracted from `CySA+ Acronyms.pdf` (included in this repo). The source document notes that terms shown in blue are "new" (not already covered in the A+, Network+, or Security+ lists) — that color-coding isn't preserved in the extracted data since it lives in PDF text color rather than text content, but every acronym, expansion, and definition is intact. Data lives in [`data/acronyms.json`](data/acronyms.json) / [`js/acronyms-data.js`](js/acronyms-data.js).

## Project structure

```
├── index.html                 # Main tile dashboard (start here)
├── data/acronyms.json         # Source data (175 entries)
├── js/
...
- Re-extract the "new to CySA+" blue-highlight flag from the PDF (via text color rather than content) and use it to filter study sessions to just the CySA+-specific terms
...
This project is provided as-is for personal, educational use. CompTIA and CompTIA CySA+ are trademarks of CompTIA, Inc. This project is an independent study aid and is not affiliated with, endorsed by, or sponsored by CompTIA.
